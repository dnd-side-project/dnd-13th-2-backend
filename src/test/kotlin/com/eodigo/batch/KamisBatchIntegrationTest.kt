...
internal class KamisBatchIntegrationTest {
    ...
    @Autowired
    private lateinit var dailyRegionalPriceRepository: DailyRegionalPriceRepository
    @Autowired
    ...
    @BeforeEach
    fun setUp() {
        regionRepository.saveAll(listOf(
            Region(regionName = "서울", regionCode = 1101),
            Region(regionName = "부산", regionCode = 2100)
        ))

        productRepository.saveAll(listOf(
            Product(name="쌀 20kg", categoryCode=100, categoryName="식량작물", itemCode=111, itemName="쌀", kindCode=1, kindName="20kg", source = ProductSource.KAMIS),
            Product(name="고랭지 배추", categoryCode=200, categoryName="채소류", itemCode=211, itemName="배추", kindCode=2, kindName="여름", source = ProductSource.KAMIS)
        ))
    }
